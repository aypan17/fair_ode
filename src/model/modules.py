    ...
    def forward(self, input_left, input_right, node_memory):
        ...
        if self.top_k > 1: 
            if not self.gate_top_k:
                ...
                memory = torch.zeros((batch_size,1,self.d_model), dtype=torch.float, requires_grad=False, device=inp.device)
                for k in range(1,self.top_k+1):
                    nn_block = getattr(self, 'pos_linear_{0}'.format(k))
                    pos_gate = nn_block(inp)
                    pos_gate = torch.sigmoid(pos_gate)
...
class BinarySMUSym(torch.nn.Module):
    ...
    def __init__(self, params):
        ...
        if params.top_k > 1:
            if not self.gate_top_k:
                self.pos_linear = torch.nn.Linear(self.d_model, params.top_k, bias=True)
            else:
                for k in range(1,self.top_k+1):
                    setattr(self, 'pos_linear_'+str(k), torch.nn.Linear(self.d_model, self.d_model, bias=True))
...
class BinaryStackSym(torch.nn.Module):
    ...
    def __init__(self, params):
        ...
        self.gate_linear_l = torch.nn.Linear(self.d_model, self.d_model, bias=True)
        self.gate_linear_r = torch.nn.Linear(self.d_model, self.d_model, bias=True)
        self.input_linear = torch.nn.Linear(self.d_model, self.d_model, bias=True)
        if params.like_LSTM:
            self.data_linear = torch.nn.Linear(self.d_model, self.d_model, bias=True)
